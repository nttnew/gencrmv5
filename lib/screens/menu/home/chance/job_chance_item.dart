    ...
    return BlocBuilder<GetJobChanceBloc, JobChanceState>(
        builder: (context, state) {
          if (state is UpdateGetJobChanceState) {
                    ...
                    child: ListView.separated(
                        itemBuilder: (context, index) {
                          return GestureDetector(
                            onTap: (){
                              AppNavigator.navigateDeatailWork(int.parse(state.data[index].id!),state.data[index].name_job??'');
                            },
            ...
            else return Center(child: WidgetText(title: "Không có dữ liệu",style: AppStyle.DEFAULT_18,),);
          }
          else
            return Container(child: Center(child: Text('Không có dữ liệu!',style: AppStyle.DEFAULT_LABEL_TARBAR,),),);
        });
  }
              ...
              children: [
                ...
                Row(
                  children: [
                    ...
                    AppValue.hSpaceTiny,
                    WidgetText(title: data.user_work_name,style: AppStyle.DEFAULT_LABEL_PRODUCT),
                  ],
                ),
                Row(
                  children: [
                    ...
                    AppValue.hSpaceTiny,
                    WidgetText(title: data.status_job??'',style: AppStyle.DEFAULT_LABEL_PRODUCT.copyWith(color: Colors.red),),
                  ],
                ),
                Row(
                  children: [
                ...
                children: [
                  ...
                  Row(
                      ...
                      SvgPicture.asset('assets/icons/Mess.svg'),
                        SizedBox(width: 3,),
                        WidgetText(title: data.total_comment.toString(),style: TextStyle(color: HexColor("#0052B4"),)
                        )]
                        )
                ],
              ),
