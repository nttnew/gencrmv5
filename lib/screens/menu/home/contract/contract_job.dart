    ...
    return Container(
        ...
        child:  BlocBuilder<JobContractBloc, JobContractState>(
            builder: (context, state) {
              if (state is SuccessJobContractState)
                if(state.listJob.length>0)
                  return ListView.separated(
                      itemBuilder: (context,index){
                        return GestureDetector(
                          onTap: (){
                            AppNavigator.navigateDeatailWork(int.parse(state.listJob[index].id!),state.listJob[index].name_job??'');
                          },
        ...
        children: [
          Row(
            ...
            children: [
              ...
              Image.asset('assets/icons/red.png',color:(data.color!=null&&data.color!="")? HexColor(data.color!):COLORS.PRIMARY_COLOR,)
            ],
          ),
          AppValue.vSpaceTiny,
          Row(
            children: [
              ...
              AppValue.hSpaceTiny,
              WidgetText(title: data.name_customer??"",style: AppStyle.DEFAULT_LABEL_PRODUCT),
            ],
          ),
          AppValue.vSpaceTiny,
          Row(
            children: [
              SvgPicture.asset('assets/icons/dangxuly.svg',color: (data.color!=null&&data.color!="")? HexColor(data.color!):COLORS.PRIMARY_COLOR,),
              AppValue.hSpaceTiny,
              WidgetText(title: data.status_job??"",style: AppStyle.DEFAULT_LABEL_PRODUCT.copyWith(color:(data.color!=null&&data.color!="")? HexColor(data.color!):COLORS.PRIMARY_COLOR),),
            ],
          ),
          AppValue.vSpaceTiny,
          Row(
            ...
            children: [
              Row(
                children: [
                  ...
                  AppValue.hSpaceTiny,
                  WidgetText(title: data.start_date??"",style: AppStyle.DEFAULT_LABEL_PRODUCT.copyWith(color: Colors.grey)),
                ],
              ),
              Row(
                children: [
                  ...
                  SizedBox(width: 5,),
                  WidgetText(title: data.total_comment.toString(),style: AppStyle.DEFAULT_12.copyWith(color: COLORS.TEXT_BLUE_BOLD),)
                ],
              )
            ],
          ),
