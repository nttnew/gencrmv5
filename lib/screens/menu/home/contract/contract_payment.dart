...
class _ContractPaymentState extends State<ContractPayment> {
  ...
  void initState() {
    ...
    //   DetailContractBloc.of(context).add(InitGetPaymentContractEvent(widget.id));
    // });
        ...
        children: [
          BlocBuilder<PaymentContractBloc, PaymentContractState>(
              builder: (context, state) {
                if (state is SuccessPaymentContractState)
                  if(state.listPaymentContract.length>0 && state.listPaymentContract!=null)
              ...
              if(data[index].field_hidden!=1)
                return Container(
                  ...
                  child: Row(
                    mainAxisAlignment: MainAxisAlignment.spaceBetween,
                    children: [
                      WidgetText(title: data[index].field_label??'',style: AppStyle.DEFAULT_14.copyWith(color: Colors.grey),),
                      WidgetText(title: data[index].field_value.toString(),style: AppStyle.DEFAULT_14)
                    ],),
                );
