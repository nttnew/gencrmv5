...
class AddNoteBloc extends Bloc<AddNoteEvent, AddNoteState>{
  ...
  AddNoteBloc({required UserRepository userRepository}) : userRepository = userRepository, super(InitAddNoteState());
  ...
  Stream<AddNoteState> mapEventToState(AddNoteEvent event) async* {
    if (event is InitAddNoteEvent) {
      ...
      yield* _editNoteCus(id: event.id,content:event.content,noteId: event.id_note,type: event.type);
    }
  ...
  Stream<AddNoteState> _addNote({required String id,required String content,required int type}) async* {
    ...
    try {
      ...
      if(type==1)
        ...
        response = await userRepository.addNoteContact(id: id,content: content);
      else if(type==3)
        response = await userRepository.addNoteOpp(id: id,content: content);
      else if(type==4)
        response = await userRepository.addNoteContract(id: id,content: content);
      else if(type==5)
  ...
  Stream<AddNoteState> _editNoteCus({required String id,required String content ,required String noteId,required int type}) async* {
    LoadingApi().pushLoading();
    try {
      yield LoadingAddNoteState();
      final response;
      if(type==1)
        response = await userRepository.editNoteCus(id: id,content: content,noteId: noteId);
      else if(type==2)
        response = await userRepository.editNoteContact(id: id,content: content,noteId: noteId);
      else if(type==3)
        response = await userRepository.editNoteOop(id: id,content: content,noteId: noteId);
      else if(type==4)
        response = await userRepository.editNoteContract(id: id,content: content,noteId: noteId);
      else if(type==5)
        response = await userRepository.editNoteJob(id: id,content: content,noteId: noteId);
      else
        response = await userRepository.editNoteSup(id: id,content: content,noteId: noteId);
      if((response.code == BASE_URL.SUCCESS)||(response.code == BASE_URL.SUCCESS_200)){
        yield SuccessAddNoteState();
      }
      else
  ...
  Stream<AddNoteState> _deleteNote({required String id,required String noteId,required int type}) async* {
    LoadingApi().pushLoading();
    try {
  ...
  static AddNoteBloc of(BuildContext context) => BlocProvider.of<AddNoteBloc>(context);
}