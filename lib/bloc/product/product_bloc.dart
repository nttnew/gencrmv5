...
class ProductBloc extends Bloc<ProductEvent, ProductState>{
  ...
  ProductBloc({required UserRepository userRepository}) : userRepository = userRepository, super(InitGetListProductState());
    ...
    try {
      ...
      if((response.code == BASE_URL.SUCCESS)||(response.code == BASE_URL.SUCCESS_200)){
        if(page=="1"){
          ...
          yield SuccessGetListProductState(response.data!.product!,response.data!.units!,response.data!.vats!,response.data!.total!);
        }
        else{
          data=[...data!,...response.data!.product!];
          yield SuccessGetListProductState(data!,response.data!.units!,response.data!.vats!,response.data!.total!);
        }
      }
      else
  ...
  static ProductBloc of(BuildContext context) => BlocProvider.of<ProductBloc>(context);
}