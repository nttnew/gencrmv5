...
class ChangePasswordBloc extends Bloc<ChangePasswordEvent, ChangePasswordState> {
  ...
  Stream<ChangePasswordState> mapEventToState(ChangePasswordEvent event) async* {
    if (event is OldPasswordChanged) {
      ...
      final newPassword = NewPassword.dirty(password: state.oldPassword.value,value: event.newPassword);
      yield state.copyWith(
        newPassword: newPassword.valid ? newPassword : NewPassword.pure(password: state.oldPassword.value),
      ...
      final newPassword = NewPassword.dirty(password:oldPass,value:state.newPassword.value);
      yield state.copyWith(
      ...
      if (state.status.isValidated) {
        ...
        try{
          var response = await userRepository.updatePass(username: shareLocal.getString(PreferencesKey.USER_NAME), oldpass: state.oldPassword.value, newpass: state.newPassword.value);
