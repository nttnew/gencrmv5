...
class GetNoteClueBloc extends Bloc<GetNoteClueEvent, NoteClueState> {
  ...
  GetNoteClueBloc({required UserRepository userRepository})
      : userRepository = userRepository,
        super(InitGetNoteClueState());
  @override
  ...
  Stream<NoteClueState> _getNoteClue(String id) async* {
    ...
    try {
      ...
      if ((responseListNoteClue.code == BASE_URL.SUCCESS) ||
          (responseListNoteClue.code == BASE_URL.SUCCESS_200)) {
  ...
  static GetNoteClueBloc of(BuildContext context) =>
      BlocProvider.of<GetNoteClueBloc>(context);
}
