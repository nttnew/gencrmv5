...
class GetListUnReadNotifiBloc extends Bloc<ListUnReadNotifiEvent, UnReadListNotifiState>{
  final UserRepository userRepository;

  GetListUnReadNotifiBloc({required UserRepository userRepository}) : userRepository = userRepository, super(InitGetUnReadListNotifiState());

  @override
  Stream<UnReadListNotifiState> mapEventToState(ListUnReadNotifiEvent event) async* {
    ...
    throw e;
      ...
      try {
        final response = await userRepository.readNotification(id:event.id,type:event.type);
        if ((response.code == BASE_URL.SUCCESS) ||
            (response.code == BASE_URL.SUCCESS_200)) {
  ...
  Stream<UnReadListNotifiState> _getListNotifi({ required int page }) async* {
    ...
    try {
      ...
      if((response.code == BASE_URL.SUCCESS)||(response.code == BASE_URL.SUCCESS_200)){
        ...
        yield UpdateUnReadListNotifiState(list:listNotifi!,total:response.data.total!,limit:response.data.limit!,page:page);
        LoadingApi().popLoading();
      }
      else{
        yield ErrorGetUnReadListNotifiState(response.msg ?? "");
  ...
  static GetListUnReadNotifiBloc of(BuildContext context) => BlocProvider.of<GetListUnReadNotifiBloc>(context);
}