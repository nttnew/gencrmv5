...
import '../../src/models/validate_form/fullname.dart';
...
class InforAccBloc extends Bloc<InforAccEvent, InforAccState> {
  ...
  InforAccBloc({required this.userRepository})
      : super(const InforAccState());
  ...
  if (event is EmailChanged) {
      ...
      yield state.copyWith(
        ...
        status: Formz.validate([state.phone,email,]),
      );
    }
      ...
      if (state.status.isValidated) {
        ...
        try{
          var response = await userRepository.changeInforAcc(fullName: event.fullName, phone: state.phone.value, email: state.email.value, address: event.address, avatar: event.avatar);
          if (response.code == BASE_URL.SUCCESS_200|| response.code == BASE_URL.SUCCESS) {
    ...
    if (state.status.isValidated) {
      ...
      try{
        var response = await userRepository.changeInforAccNoAvatar(fullName: event.fullName, phone: state.phone.value, email: state.email.value, address: event.address);
        if (response.code == BASE_URL.SUCCESS_200|| response.code == BASE_URL.SUCCESS) {
          yield state.copyWith(status: FormzStatus.submissionSuccess, message: response.msg);
        } else {
          yield state.copyWith(status: FormzStatus.submissionFailure, message: response.msg);
  ...
  static InforAccBloc of(BuildContext context) => BlocProvider.of<InforAccBloc>(context);
}
